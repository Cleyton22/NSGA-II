...
// Função para verificar se um vértice foi removido da solução durante a mutação
...
Population* secondaryMainLoopToNsga2(Population *nextGeneration, ListDoubly *nonDominatedSolutions, Edge *allEdges, int numVertices){
...
void nsga2MainLoop(Edge *allEdges, int numVertices, int amountOfAllEdges){
    ...
    for (int i = 0; i < GENERATION_AMOUNT; i++){
        nextGeneration = secondaryMainLoopToNsga2(nextGeneration, nonDominatedSolutions, allEdges, numVertices);
    }
    ...
    printf("NSGA-II levou %f segundos para executar \n", cpuTimeUsed);
}
...
int main() {
    ...
    nsga2MainLoop(allEdges, numVertices, amountOfAllEdges);
